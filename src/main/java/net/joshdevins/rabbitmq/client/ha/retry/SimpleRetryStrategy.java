...
public class SimpleRetryStrategy implements RetryStrategy {
	...
	private static final Logger LOG = Logger
			.getLogger(SimpleRetryStrategy.class);
	...
	public boolean shouldRetry(final Exception e,
		...
		if (LOG.isDebugEnabled()) {
			LOG
					.debug("Operation invocation failed on IOException: numOperationInvocations="
		...
		if (numOperationInvocations == maxOperationInvocations) {
			...
			if (LOG.isDebugEnabled()) {
				LOG
						.debug("Max number of operation invocations reached, not retrying: "
								+ maxOperationInvocations);
			}
		...
		if (operationRetryTimeoutMillis > 0) {
			...
			if (LOG.isDebugEnabled()) {
				LOG
						.debug("Sleeping before next operation invocation (millis): "
